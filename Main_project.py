            ...
            for character_of_transition1 in first_transducer.transitions[initial_state_of_transition1]:
                for character_of_transition2 in other_transducer.transitions[initial_state_of_transition2]:
                    if character_of_transition1 == character_of_transition2:
                        if (initial_state_of_transition1, initial_state_of_transition2) not in transitions:
                            transitions[(initial_state_of_transition1, initial_state_of_transition2)] = dict()
                        if character_of_transition1 not in transitions[(initial_state_of_transition1, initial_state_of_transition2)]:
                            transitions[(initial_state_of_transition1, initial_state_of_transition2)][character_of_transition1] = dict()
                        ...
                        for character_changed1 in first_transducer.transitions[initial_state_of_transition1][
                            ...
                            for character_changed2 in other_transducer.transitions[initial_state_of_transition2][character_of_transition2]:
                                if (character_changed1, character_changed2) not in transitions[(initial_state_of_transition1, initial_state_of_transition2)][character_of_transition1]:
                                    transitions[(initial_state_of_transition1, initial_state_of_transition2)][character_of_transition1][(character_changed1, character_changed2)] = set()
...
def find_born_of_number(square_transducer):
...
def T1_criteria(square_tranducer, dag_set_state):
    ...
    if len(dag_set_state) != 1:
        for e in dag_set_state:
            ...
            if square_tranducer.initial_states in list_of_state:
        ...
        if state not in already_marked:
            if type(state[0]) != str:
                                    ...
                                    if element1 in square_transducer.transitions[element][character_of_transition][
                                        ...
                                        if character_changed != [('', '')]:
                                            ...
                                            for state_co_accessible in set_of_co_accessible:
                                                if type(state_co_accessible[0]) == str:
                                                    marked_state.add(state_co_accessible)
                ...
                if element in square_transducer.transitions:
                    for character_of_transition in square_transducer.transitions[element]:
                        for character_changed in square_transducer.transitions[element][character_of_transition]:
                                if element in square_transducer.transitions[element][character_of_transition][
                                    character_changed]:
                                    if character_changed != [('', '')]:
                                        marked_state.add(element)
                                        already_marked.add(state)
                                        set_of_co_accessible = ascend_edges(dag, state)
                                        for state_co_accessible in set_of_co_accessible:
                                            if type(state_co_accessible[0]) == str:
...
def creat_random_transducer3(number_of_state_max, density_transition, number_of_final_state, input_symbols,
    ...
    out = set()
    number_of_transition = int(
        number_of_state_max * (number_of_state_max -1) * len(input_symbols) * density_transition)
            ...
            for character_to_changed in graph.transitions[initial]:
                for character_changed in graph.transitions[initial][character_to_changed]:
                    for final_state_of_transition in graph.transitions[initial][character_to_changed][character_changed]:
                        new_element = (elem[0],elem[1] + character_to_changed,elem[2] + character_changed,final_state_of_transition)
...
def mofif_graph(state_to_parcourt2,graph_state,graph_transition1,born):

    graph_transition = graph_transition1
        ...
        for elem2 in state_to_parcourt2:
            if elem[1] == elem2[1]:
                ...
                for e in state_to_go_from:
                    if e[3] == 1:
                        if abs(e[2] + len(elem[2]) - len(elem2[2])) <= born:
                            new_element = (elem[3], elem2[3], e[2] + len(elem[2]) - len(elem2[2]), 1, e[4])
                            graph_transition[e].add(new_element)
                        if e[4] in elem[2]:
                            for let in range(len(elem[2])):
                                if elem[2][let] == e[4]:
                                    j = let
                                    if abs(e[2] + j + 1 - len(elem2[2])) <= born and e[2] + j + 1 - len(elem2[2]) > 0:
                                        new_element = (elem[3], elem2[3], e[2] + j + 1 - len(elem2[2]), 2, e[4])
                                        graph_transition[e].add(new_element)
                                    if e[2] + j <len(elem2[2]):
                                        if e[2] + j >=0 :
                                             if elem2[2][e[2] + j] != e[4]:
                                                new_element = (elem[3], elem2[3], 0, 3, e[4])
                                                graph_transition[e].add(new_element)
                    if e[3] == 2:
                        if e[2] - len(elem2[2]) + 1 > 0:
                            if abs(e[2] - len(elem2[2]) + 1) <= born:

                                new_element = (elem[3], elem2[3], e[2] - len(elem2[2]) + 1, 2, e[4])
                                graph_transition[e].add(new_element)

                        if e[2] <= len(elem2[2]) - 1:
                            if e[2] >= 0:
                                if elem2[2][e[2]] != e[4]:
                                    new_element = (elem[3], elem2[3], 0, 3, e[4])
                                    graph_transition[e].add(new_element)
...
def T2_criteria(square_transducer, transducer, element_marked,dag):
    born = int(find_born_of_number(square_transducer))
    max_len1 = 0
    max_len2 = -1
    max_len = 0
    for i in dag:
    ...
    graph_state = []
    initial_to_go_from = set()
    already_visited = set()
    alphabet = find_alphabete_for_T2(transducer.output_symbols)
    for let in alphabet:
        initial_to_go_from.add((square_transducer.initial_states[0],square_transducer.initial_states[1],0,1,let))
    list_to_attein = dict()
                ...
                for index in range(-born,born+1):
                    for i in range(1,4):
                        for letter in alphabet:
                            graph_state += [(states,states2,index,i,letter)]

    for truc in initial_to_go_from:
        list_to_attein[truc] = set()
        for state in element_marked:
                list_to_attein[truc].add((state[0],state[1],0,3,truc[4]))
    ...
    graph_transition = mofif_graph(state_to_parcourt1, graph_state, graph_transition, born)


    for e in initial_to_go_from:
        list_of_accessible = depth_find_search(graph_transition, e)
        for e2 in list_of_accessible:
            if e2 in list_to_attein[e]:
                return False


    for i in range(len(transducer.states)- max_len1  + max_len):
...
class transducer:
    ...
    def trim(self):
        ...
        states = states_set_of_trim_nfa(nfa_from_transducer)
        nfa_from_transducer = sub_automaton(nfa_from_transducer, states)
        self.compare_nfa_and_transducer(nfa_from_transducer)
        self.states = states
        self.final_states = self.final_states.intersection(states)
    def is_sequential(self, step_by_step_print=False,want_t1 = False):
        """
        ...
        automaton = from_transducer_to_multiple_initial_nfa(square_transducer)
        if want_t1:
            dag_state = find_marked_states_for_dag(automaton)
            if not T1_criteria(square_transducer, dag_state):
                return False
                                ...
                                if h_prime == 0:
                                    ...
                                    if case_2:
                                        ...
                                        if are_comparable(h_prime[1], T2[final_state_of_transition]):
                                            if len(h_prime[1]) > len(T2[final_state_of_transition]):
        ...
        return True
    def is_sequential_weeber_and_klem(self):
        self.trim()
        automaton2 = from_transducer_to_multiple_initial_nfa(self)
        dag_state2 = find_marked_states_for_dag(automaton2)
        square = square_transducer_product(self, self)
        automaton = from_transducer_to_multiple_initial_nfa(square)
        dag_state = find_marked_states_for_dag(automaton)
        set_of_marked_state =mark_state(square)
        set_of_co_accessible_state_from_circle = states_set_of_co_accessible_nfa(automaton, set_of_marked_state)
        if T1_criteria(square, dag_state):
            if T2_criteria(square,self,set_of_co_accessible_state_from_circle,dag_state2):
    ...
    def is_function(self, as_been_visited=None):
        """
        Find if the transducer is functionnal or not.
...
transduce = transducer(
    ...
    input_symbols={'a','b'},
    initial_states='q2',
    output_symbols={'f','o','m','s','t','k','s','w','b','h','n','y'},
    final_states={'q2', 'q0', 'q1'},
    transitions={'q4': {'a': {'f': {'q1'}}, 'b': {'o': {'q2'}}}, 'q1': {'b': {'m': {'q0'}, 's': {'q3'}}, 'a': {'t': {'q3'}, 'k': {'q0'}}}, 'q0': {'a': {'s': {'q4'}, 'w': {'q3'}}, 'b': {'b': {'q2'}}}, 'q2': {'a': {'h': {'q1'}}, 'b': {'n': {'q3'}}}, 'q3': {'a': {'y': {'q1'}}}}
)
print(transduce.is_sequential())
print(transduce.is_sequential_weeber_and_klem())
"""
